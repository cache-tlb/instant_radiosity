    ...
    while(stackptr>=0) {
        ...
        if( node.rightOffset == 0 ) {
            for(uint32_t o=0;o<node.nPrims;++o) {
                ...
                const Shape* obj = (*build_prims)[node.start+o];
...
BVH::BVH(std::vector<Shape*>* objects, uint32_t leafSize)
...
void BVH::Build() {
    ...
    while(stackptr > 0) {
        ...
        float split_coord = .5f * (bc.min_p[split_dim] + bc.max_p[split_dim]);
