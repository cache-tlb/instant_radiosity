...
void main() 
{
    ...
    vec3 rvec = normalize(vec3(rand(vUv.x + pos.x), rand(vUv.y + pos.y), 0.0));
    ...
    for (int i = 0; i < num_sample; i++) {
        ...
        vec4 proj_sample_pos = m_projection * m_view * vec4(sample_pos, 1.0);
        vec4 sample_view = m_view * vec4(sample_pos, 1.0);
